...
#include "memory.h"
char buffer[BUFFER_SIZE]; // 3456 bytes
extern int selector;
char c;
char epuck_id[4];
int epuck_index = 0;
...
int main() {
	...
	e_init_uart2(BAUD230400);   // initialize UART to 115200 Kbaud
	...
	if (selector==0) {
		...
		run_asercom(USE_UART_1);		// advanced sercom protocol
